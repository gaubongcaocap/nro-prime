...
public class ItemService {
    ...
    public void OpenSKH(Player player, int itemUseId, int select) throws Exception {
        ...
        int[][] options = {{128, 129, 127, 120, 121}, {130, 131, 132, 120, 121}, {133, 135, 134, 120, 121}};
        int skhv1 = 25;// ti le
        int skhv2 = 35;//ti le
        int skhc = 40;//ti le
        int skhId = -1;
...
///////////////random skh 
    ...
    public int SKHC2(byte gender) {
        ...
        int skhv1 = 35;  // Tỉ lệ xác suất cho sự kiện 1 (20%)
        int skhv2 = 35;  // Tỉ lệ xác suất cho sự kiện 2 (20%)
        int skhv3 = 30;  // Tỉ lệ xác suất cho sự kiện 3 (20%)
    ...
    public void setthuongde(Player player) throws Exception {
        Item hq = InventoryServiceNew.gI().findItem(player.inventory.itemsBag, 1475);
        ...
        ao.itemOptions.add(new Item.ItemOption(210, 1));
        quan.itemOptions.add(new Item.ItemOption(210, 1));
        gang.itemOptions.add(new Item.ItemOption(210, 1));
        giay.itemOptions.add(new Item.ItemOption(210, 1));
        nhan.itemOptions.add(new Item.ItemOption(210, 1));
        ao.itemOptions.add(new Item.ItemOption(213, 1));
        quan.itemOptions.add(new Item.ItemOption(213, 1));
        gang.itemOptions.add(new Item.ItemOption(213, 1));
        giay.itemOptions.add(new Item.ItemOption(213, 1));
        nhan.itemOptions.add(new Item.ItemOption(213, 1));
        ...
        if (InventoryServiceNew.gI().getCountEmptyBag(player) > 4) {
            ...
            InventoryServiceNew.gI().sendItemBags(player);
            Service.getInstance().sendThongBao(player, "Bạn đã nhận được set Thượng Đế");
    ...
    public Item fusionSKH(int itemId, int skhId) {
        Item item = createItemSetKichHoat(itemId, 1);
        if (item != null) {
            item.itemOptions.addAll(ItemService.gI().getListOptionItemShop((short) itemId));
    ...
    public int pagramItemSKH(int typeItem) {
        switch (typeItem) {
            case 0:
            case 2:
                return Util.nextInt(5); //+5 giáp +5 tân công
            case 1:
            case 3:
                return Util.nextInt(20, 30); //20-30 hp và ki
            default:
                return Util.nextInt(3); //chí mạng
        }
    }
    /////
    ...
    public int optionIdSKHFusion(int skhId) {
        switch (skhId) {
            case 235: //Set Viet Taiyoken
                return 238;
            case 236: //Set Viet Genki
                return 239;
            case 237: //Set Viet Kamejoko
                return 240;
        }
        return 0;
    }
