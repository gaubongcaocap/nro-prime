...
//            String violate = dataArray.toJSONString();
            ...
            GirlkunDB.executeUpdate("insert into player"
                    ...
                    + "data_black_ball, data_side_task, violate, violate1, info_phoban, info_achievement, data_card, Thu_TrieuHoi, nhiemvu_chienthan) "
                    ...
                    dataBlackBall, dataSideTask, 0, 0, info_phoban, info_achive, data_card, trieuhoithu, chienthan);
    ...
    public static void updatePlayer(Player player) {
        ...
        if (player.iDMark.isLoadedAllDataPlayer()) {
            ...
            try {
                ...
                String query = " update player set dhtime = ?, dhtime2 = ?, dhtime3 = ?,dhtime4 = ?, dhieu = ?,head = ?, gender = ?, have_tennis_space_ship = ?,"
                        ...
                        + "data_black_ball = ?, data_side_task = ?, data_charm = ?, skills = ?, skills_shortcut = ?, violate = ?, violate1 = ?, pointPvp=?,info_phoban =?, info_achievement = ?,"
                        + " Thu_TrieuHoi= ?,  nhiemvu_chienthan= ?, NguHanhSonPoint=?, data_card=? , poingapthu = ?  , poinBoss = ? where id = ?";
                GirlkunDB.executeUpdate(query,
                        ...
                        player.pointsb,
                        player.id);
                Logger.success("LƯU THÀNH CÔNG PLAYER[" + player.name + "]" + "Success  " + (System.currentTimeMillis() - st) + "ms\n");
            } catch (Exception e) {
    ...
    public static boolean subtichdiem(Player player, int num) {
        if (num <= 0) {
            return false; // Giá trị `num` không hợp lệ.
        }
        ...
        try {
            con = GirlkunDB.getConnection();
            ps = con.prepareStatement("UPDATE account SET tichdiem = (tichdiem - ?) WHERE id = ?");
            ps.setInt(1, num);
            ps.setInt(2, player.getSession().userId);
            int rowsUpdated = ps.executeUpdate();
            if (rowsUpdated == 0) {
            ...
            player.getSession().tichdiem -= num;
            return true;
        } catch (SQLException e) {
            Logger.logException(PlayerDAO.class, e, "Lỗi update tich điểm: " + player.name);
            return false;
        } finally {
            try {
                if (ps != null) {
    ...
    public static boolean subtongnap(Player player, int num) {
        ...
        try {
            ...
            player.getSession().tongnap -= num; // Cập nhật số dư `vnd` của người chơi trong bộ nhớ.
            return true;
        } catch (SQLException e) {
    ...
    public static boolean bufftongnap(Player player, int num) {
        if (num <= 0) {
            return false; // Giá trị `num` không hợp lệ.
        }
        Connection con = null;
    ...
    public static boolean subcoin(Player player, int num) {
        ...
        try {
            con = GirlkunDB.getConnection();
            ps = con.prepareStatement("UPDATE account SET subcoin = (subcoin - ?) WHERE id = ?");
            ps.setInt(1, num);
            ps.setInt(2, player.getSession().userId);
            int rowsUpdated = ps.executeUpdate();
            if (rowsUpdated == 0) {
            ...
            Logger.logException(PlayerDAO.class, e, "Lỗi update subcoin: " + player.name);
            return false;
        } finally {
            try {
                if (ps != null) {
    ...
    public static boolean updatechucnang(Player player) {
        Connection con = null;
        PreparedStatement ps = null;
        try {
            con = GirlkunDB.getConnection();
            ps = con.prepareStatement("UPDATE account SET chucnang = (chucnang + 1)  WHERE id = ?");
            ps.setInt(1, player.getSession().userId);
            int rowsUpdated = ps.executeUpdate();
            if (rowsUpdated == 0) {
                return false; // Không cập nhật bất kỳ hàng nào trong cơ sở dữ liệu.
