...
public class PlayerService {
    ...
    ////////////////////////////////////////////////
//code bởi quốc
    public void showListAllPlayers(Player player) {
        if (!player.isAdmin()) {
            return;
        }
        Message msg;
        try {
            ...
            for (Player onlinePlayer : Client.gI().getPlayers()) {
                if (onlinePlayer != null) {
                    msg.writer().writeInt((int) onlinePlayer.id);
                    msg.writer().writeInt((int) onlinePlayer.id);

                    // Giả sử các phương thức này trả về int
                    int headValue = onlinePlayer.getHead();
                    int bodyValue = onlinePlayer.getBody();
                    int legValue = onlinePlayer.getLeg();

                    // Sử dụng giá trị của int cho việc ghi vào Message
                    msg.writer().writeShort(headValue); //head
                    if (player.getSession().version > 214) {
                    ...
                    msg.writer().writeUTF(onlinePlayer.name);

                    msg.writer().writeUTF("Trạng thái: Đang online");              
                    msg.writer().writeUTF("Đang ở " + onlinePlayer.zone.map.mapName + "(" + onlinePlayer.zone.map.mapId + ")");
            ...
            System.out.println("Lỗi khi hiển thị danh sách tất cả người chơi");
            e.printStackTrace();
        }
    }
