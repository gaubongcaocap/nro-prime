...
public class Trade {
    ...
    public void addItemTrade(Player pl, byte index, int quantity) {
        // Kiểm tra xem index có phải là -1 không (đánh dấu giao dịch vàng)
        if (index == -1) {
            // Thiết lập số lượng vàng cho người chơi tương ứng
            ...
            // Xử lý giao dịch vật phẩm
            Item item = null;
            // Xác định vật phẩm dựa trên người chơi
            if (pl.equals(this.player1)) {
            ...
            // Kiểm tra xem vật phẩm có thể giao dịch không và loại bỏ nếu cần
            if (isItemCannotTran(item) == true) {
                removeItemTrade(pl, index);
            } else {
                // Xử lý logic giao dịch vật phẩm dựa trên số lượng
                if (quantity > 99) {
                    // Chia số lượng thành các lô có 99 phần tử
                    int n = quantity / 99;
                    ...
                    for (int i = 0; i < n; i++) {
                        ...
                        // Cập nhật hành trang và danh sách vật phẩm giao dịch của người chơi
                        if (pl.equals(this.player1)) {
                    ...
                    // Xử lý giao dịch cho số lượng nhỏ hơn hoặc bằng 99
                    Item itemTrade = ItemService.gI().copyItem(item);
    ...
    private void removeItemTrade(Player pl, byte index) {
        ...
        try {
            ...
            pl.sendMessage(msg);
            msg.cleanup();
            Service.getInstance().sendThongBao(pl, "Mày đang giao dịch hàng cấm đấy");
        } catch (Exception e) {
        }
    }
