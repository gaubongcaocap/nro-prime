...
public class Lancon extends Boss {
    ...
    public Lancon(int bossID, BossData bossData, Zone zone, int x, int y) throws Exception {
    ...
    public void active() {
        ...
        if (Util.getDistance(playerTarger, this) > 500 && this.zone == this.playerTarger.zone) {
            Service.gI().sendThongBao(this.playerTarger, "|7|Người không dẫn theo kì lân à");
        ...
        if (Util.canDoWithTime(this.lastTimeAttack, 4000)) {
            try {
                Service.gI().chat(this, playerTarger.name + "\n|3|Hãy đưa ta đến " + MapService.gI().getMapById(this.mapHoTong).mapName + " để gặp mẹ ta");
            } catch (Exception ex) {
