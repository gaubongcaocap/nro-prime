...
public class GodGK {
    ...
    public static Boolean editgame = false;
    ...
    public static synchronized Player login(MySession session, AntiLogin al) {
        ...
        try {
            ...
            if (rs.first()) {
                ...
                session.tichdiem = rs.getInt("tichdiem");
                session.chucnang = rs.getInt("chucnang");
...
//                    Service.getInstance().sendThongBaoOK(session, "Tài khoản đang được đăng nhập tại máy chủ khác");
                    ...
                    if (secondsPass < Manager.SECOND_WAIT_LOGIN) {
                        ...
                        if (!rs.first()) {
                            ...
                            player.diemdanh1 = rs.getLong("violate1");
                            player.pointPvp = rs.getInt("pointPvp");
                            player.gtPoint = rs.getInt("poingapthu");
                            player.pointsb = rs.getInt("poinBoss");
                            ...
                            if (dataArray.size() > 13) {
                                player.numKillSieuHang = Long.parseLong(String.valueOf(dataArray.get(14)));
                                player.rankSieuHang = Long.parseLong(String.valueOf(dataArray.get(13)));
...
//                            int timeMayDo2 = Integer.parseInt(String.valueOf(dataArray.get(10)));    
                            ...
                            if (!petData.isEmpty()) {
                                ...
                                pet.level = Byte.parseByte(String.valueOf(dataArray.get(6)));
        ...
        try {
            ...
            if (rs.first()) {
                ...
                if (clanId != -1) {
                    ...
                    } catch (ClanService.ClanNotFoundException e) {
                        // Xử lý trường hợp không tìm thấy clan
                        // Ví dụ: Log thông báo hoặc thực hiện xử lý khác tùy thuộc vào yêu cầu của ứng dụng
                        System.out.println("Không tìm thấy clan: " + e.getMessage());
                    } catch (Exception ex) {
