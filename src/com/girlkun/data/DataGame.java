...
public class DataGame {
    ...
    public static void effData(MySession session, int id, int... idtemp) {
        int idT = idtemp.length > 0 && idtemp[0] != 0 ? idtemp[0] : id;

        if ((id == 210 || id == 205 || id == 206 || id == 207) && session != null && session.version > 228) {
            sendEffectTemplate(session, id);
            return;
        }

        try {
            String dataFilePath = "data/girlkun/effect/x" + session.zoomLevel + "/data/DataEffect_" + idT;
            String imgFilePath = "data/girlkun/effect/x" + session.zoomLevel + "/img/ImgEffect_" + idT + ".png";

            byte[] effData = FileIO.readFile(dataFilePath);
            byte[] effImg = FileIO.readFile(imgFilePath);

            if (effData != null && effImg != null) {
                Message msg = new Message(-66);
...
//                System.out.println("Error: Unable to read files for id " + idT);
            }
        } catch (Exception e) {
            Logger.logException(DataGame.class, e);
            System.out.println("Error: Exception occurred for id " + idT);
        }
    }
    ...
    public static void sendItemBGTemplate(MySession session, int id) {
        ...
        try {
            ...
            if (bg_temp != null) {  // Kiểm tra nếu bg_temp không phải là null
                msg = new Message(-32);
                msg.writer().writeShort(id);
                ...
                System.out.print(RED + "Lỗi 7: Tệp tin không tồn tại hoặc có lỗi khi đọc" + RESET + "\n");
            }
        } catch (Exception e) {
            Logger.logException(DataGame.class, e);
    ...
    public static void sendDataItemBG(MySession session) {
        try {
            byte[] itemBgData = FileIO.readFile("data/girlkun/item_bg_temp/item_bg_data");

            if (itemBgData != null) {
                Message msg = new Message(-31);
                msg.writer().write(itemBgData);
                session.sendMessage(msg);
                msg.cleanup();
            } else {
                System.out.print(RED + "Lỗi 8: Tệp tin không tồn tại hoặc có lỗi khi đọc" + RESET + "\n");
            }
        } catch (Exception e) {
            Logger.logException(DataGame.class, e);
            System.out.println("Lỗi 8: " + e.getMessage());
        }
    }

//    //anti keors
//    public static void antiKeoRes(MySession session, short from, short to) {    
//        System.out.println("-------Da kick thang ngu kéo res  :   " + session.ipAddress);
...
//        }
    ...
    public static void sendImageByName(MySession session, String imgName) {
        ...
        try {
            ...
            // Đảm bảo rằng data không phải là null trước khi sử dụng
            ...
            if (data != null) {
                ...
                System.out.println("Data is null for image: " + imgName);
            }
        } catch (IOException e) {
            System.out.println("Error loading image by name: " + imgName + " - x" + session.zoomLevel);
            e.printStackTrace();
        }
    }
    ...
    public static void sendVersionRes(ISession session) {
        ...
        try {
            ...
            System.out.println("xảy ra tại lỗi 17");
        }
    }
        ...
        try {
            ...
            if (resFolder.exists() && resFolder.isDirectory()) {
                ...
                if (files != null) {
                    ...
                    msg.writer().writeShort(fileCount);
                    session.sendMessage(msg);
                    msg.cleanup();
                } else {
                    System.out.println("Không thể đọc danh sách tệp trong thư mục res.");
                }
            } else {
                System.out.println("Thư mục res không tồn tại hoặc không phải là một thư mục.");
            }
        } catch (Exception e) {
    ...
    public static void sendRes(MySession session) {
        try {
            ...
            Message finalMsg = new Message(-74);
            finalMsg.writer().writeByte(3);
            finalMsg.writer().writeInt(vsRes);
            ...
            // Error occurs due to data corruption; entering the game again should not be affected
        }
    }
