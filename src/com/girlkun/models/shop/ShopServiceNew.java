...
public class ShopServiceNew {
    ...
    private static final byte SHOP_24h = 6;
    ...
    public void opendShop(Player player, String tagName, boolean allGender) {
        ...
        try {
            ...
            ex.printStackTrace();
            System.out.println("lỗi nè     ClassCastException ");
    ...
    public Shop resolveShopBua(Player player, Shop s) {
    ...
    public void openShopType4(Player player, String tagName, List<Item> items) {

        if (items == null) {
            return;
        }

        // Đặt tên thẻ cho đánh dấu ID của người chơi
        player.iDMark.setTagNameShop(tagName);

        // Tạo một tin nhắn mới để gửi cho người chơi
        Message msg;
        try {
            // Khởi tạo tin nhắn với một mã cụ thể
            msg = new Message(-44);

            // Ghi thông tin về loại cửa hàng và tên vào tin nhắn
            msg.writer().writeByte(4);
            msg.writer().writeByte(1);
            msg.writer().writeUTF("Kho\nGắp Thú");

            // Ghi số lượng mặt hàng trong cửa hàng
            msg.writer().writeByte(items.size());

            // Vòng lặp qua từng mặt hàng để ghi thông tin chi tiết
            for (Item item : items) {

                // Ghi ID mẫu của mặt hàng
                msg.writer().writeShort(item.template.id);
                msg.writer().writeUTF("\n|7|Chào mừng đến với ngọc rồng Nro");

                // Ghi số lượng tùy chọn của mặt hàng và các giá trị
            ...
            player.sendMessage(msg);
            // Dọn dẹp tài nguyên sau khi gửi tin nhắn
            msg.cleanup();
        } catch (Exception e) {
            // Xử lý ngoại lệ và in thông báo nếu có lỗi
            System.out.println("bbbbb");
        }
    }

    public void openShopType6(Player player, Shop shop, List<Item> items) {
        player.iDMark.setShopOpen(shop);
        player.iDMark.setTagNameShop(shop.tagName);
        if (items == null) {
            return;
     ...
     */
    public void buyItemBua(Player player, int itemTempId) {
        Shop shop = player.iDMark.getShopOpen();
    ...
    public void buyItem(Player player, int itemTempId) {
        ...
        if (is.temp.id >= 560 && is.temp.id <= 662) {
            if (is.temp.type == 0) {
                Item it = player.inventory.itemsBody.get(0);
                InventoryServiceNew.gI().subQuantityItemsBody(player, it, 1);
    ...
    public boolean subIemByItemShop(Player pl, ItemShop itemShop) {
        boolean isBuy = false;
    ...
    public void showConfirmSellItem(Player pl, int where, int index) {
        ...
        if (item != null && item.isNotNullItem() && item.template.id == 457) {
            NpcService.gI().createMenuConMeo(pl, ConstNpc.BAN_NHIEU_THOI_VANG, 7710,
                    ...
                    + "|-1|Theo nguyện vọng góp ý từ các chiến binh, ta được Admin Ngọc Rồng Nro\n"
    ...
    public void getItemSideBoxLuckyRound(Player player, List<Item> items, byte type, int index) {
