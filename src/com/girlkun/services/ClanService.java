...
public class ClanService {
    ...
    public Clan getClanById(int id) throws ClanNotFoundException {
        return getClanById(0, Manager.CLANS.size() - 1, id);
    }

    private Clan getClanById(int l, int r, int id) throws ClanNotFoundException {
        if (l <= r) {
            int m = (l + r) / 2;
            if (m >= 0 && m < Manager.CLANS.size()) {
                Clan clan = Manager.CLANS.get(m);
                if (clan.id == id) {
                    return clan;
                } else if (clan.id > id) {
                ...
                throw new ClanNotFoundException("Không tìm thấy clan id: " + id);
            }
        } else {
            throw new ClanNotFoundException("Không tìm thấy clan id: " + id);
        }
