...
//                    }
                ...
                Service.getInstance().sendThongBao(player, "Bớt báo hộ ad cái !!!");
            }
        }
    ...
    public void itemBagToBody(Player player, int index) {
        ...
        if (item.isNotNullItem()) {
            ...
            if (item.template.id > 1299 && item.template.id < 1309) { //danh hiệu
    ...
    public boolean addItemBag(Player player, Item item) {
        ...
        if (item.template.id == 517) {
            if (player.inventory.itemsBag.size() < Inventory.MAX_ITEMS_BAG) {
                ...
                Service.getInstance().sendThongBao(player, "Hành trang của bạn đã được mở rộng thêm 1 ô");
                return true;
            } else {
                Service.getInstance().sendThongBao(player, "Hành trang của bạn đã đạt tối đa");
                return false;
            }
            ...
            if (player.inventory.itemsBox.size() < Inventory.MAX_ITEMS_BOX) {
                ...
                Service.getInstance().sendThongBao(player, "Rương đồ của bạn đã được mở rộng thêm 1 ô");
                return true;
            } else {
