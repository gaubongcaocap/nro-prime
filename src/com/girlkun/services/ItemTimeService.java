...
public class ItemTimeService {
    ...
    public void sendAllItemTime(Player player) {
        ...
        if (player.itemTime.isUseMayDoSC) {
            sendItemTime(player, 30153, (int) ((TIME_MAY_DO - (System.currentTimeMillis() - player.itemTime.lastTimeUseMayDoSC)) / 1000));
        }
        ...
        if (player.itemTimesieucap.isKeo) {
            sendItemTime(player, 20177, (int) ((TIME_KEO - (System.currentTimeMillis() - player.itemTimesieucap.lastTimeKeo)) / 1000));
        }      
        if (player.itemTimesieucap.isUseXiMuoi) {
            sendItemTime(player, 18228, (int) ((TIME_XI_MUOI - (System.currentTimeMillis() - player.itemTimesieucap.lastTimeUseXiMuoi)) / 1000));
        }
    ...
    public void sendCanAutoPlay(Player player) {
        ...
        try {
            msg = new Message(-116);
            msg.writer().writeByte(player.autodosat == true ? 1 : 0);
