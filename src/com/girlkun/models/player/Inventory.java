...
public class Inventory {
    ...
    // lấy option của món đồ
    public int getOption(Item item, int index, int id) {
        if (item != null && index >= 0 && index < item.itemOptions.size()) { // Kiểm tra item và index hợp lệ
            ItemOption option = item.itemOptions.get(index); // Lấy tùy chọn tại vị trí index
            if (option != null && option.optionTemplate.id == id) { // Kiểm tra tùy chọn có tồn tại và có id tương ứng không
                return option.param; // Trả về giá trị tham số của tùy chọn
            }
        }
        return 0; // Trả về giá trị mặc định nếu không tìm thấy tùy chọn có id tương ứng
    }
